...
### 5.3 当前状态（2026-08-07 冻结点 `edd54e8` 实测，已归档）

harness 落地于 `scripts/bench/`；aicir / Qiskit 2.4.2 / qiskit-aer 0.17.2 /
Cirq 1.7.0 四方 parity 16/16 通过，门数亦已对齐（QFT 四家同为 136 门）。
全量清单归档在 `docs/evidence/benchmarks/edd54e86.../cpu.json`（250 条记录、
8 个轴、`release_gate=PASS`、`worktree_dirty=false`）。

**本节此前的结论已被推翻，如实更正**：2026-08-06 版写的是"与 Qiskit 大致持平、
与 Cirq 有真实差距"。装上 qiskit-aer（真正的高性能 CPU 基线）后，n=18 执行时间
中位数（秒）为：

| 线路族 | aicir | qiskit-aer | cirq | qiskit |
| --- | --- | --- | --- | --- |
| ghz | 0.01653 | 0.01387 | **0.00584** | 0.01706 |
| qft | 0.18069 | 0.04612 | **0.02021** | 0.17309 |
| random | 0.13874 | **0.02351** | 0.04137 | 0.09810 |
| layered_ansatz | 0.27361 | **0.05340** | 0.06440 | 0.19452 |

**aicir 稳定快于 Qiskit 参考 `Statevector`，但慢于 Aer 与 Cirq**（对 Aer 约
1.2–4×，对 Cirq 约 2.8–9×，随比特数扩大）。"CPU 可信"只能相对 Qiskit 参考
实现成立，**不能写成与 Aer/Cirq 平齐**。

差距已定位，且不在算术内核（单比特门内核实测快于 Cirq 的 `apply_unitary`）：
一是 Cirq 默认开启 `split_untangled_states`（关掉后 Cirq 变慢 1.14–1.97×），
那是**另一个算法**而非调参；二是本方门应用对半态做了 6 趟而下界是 2 趟，
约 3 倍多余访存。可得的优化（缓冲复用 1.23×、对角门专用 2.6×、按几何选核最高
4.3×）复合后只能把与 Cirq 的差距收到约 2–4×，**达不到平齐**——平齐需要引入
因子化态表示，那是第二个模拟引擎而非优化 pass（已作为 v2 落在
`feat/factored-engine` 分支，不进 `paper-v1`）。

**§8 的框架因此调整**：不再主张"CPU 平价"，而是"同一数量级 + 别处独有能力"，
论证重心移到轴 G（能力矩阵）。A–D 用于建立可信度并如实报告输掉的行。

轴 E（昇腾）已有两份 commit 钉死的真机清单，见 §8。
仍缺：轴 C 的跨框架 VQE 封装、轴 F 的多卡绝对加速。
