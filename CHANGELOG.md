...
- **`aicir.optimization.sb` 弃用预告。** 占位包（无公开 API）从 `aicir.optimization.__init__` 的自动子模块导出中移除（`aicir.optimization.__all__` 不再含 `sb`）；仍可 `import aicir.optimization.sb` 显式导入，但会触发 `DeprecationWarning`，计划在后续版本删除该包。
