...
### 昇腾真机实测（commit `4c0c176`，`npu:0`，`fallback_to_cpu=false`）

`scripts/npu/pauli_sparse.sh` 8/8 通过。正确性误差 `1.7e-8`（complex64 精度）：
态矢量 `1.72e-8`、含 Y 串 `2.18e-8`、密度矩阵 `1.71e-8`——两处符号约定在真机上均正确。

| n | 单次期望 | 稠密 H 需要 |
| --- | --- | --- |
| 14 | 17.87 ms | 2.15 GB |
| 16 | 20.58 ms | 34.36 GB |
| 18 | 24.12 ms | 549.76 GB |
| 20 | 32.34 ms | **8796 GB** |

**关键观察：耗时与项数成正比，与态大小无关。** n=14→20 振幅数增长 64×，
耗时只增长 1.81×，而项数增长 1.44×；每项耗时 0.66 / 0.66 / 0.69 / 0.83 ms 近乎恒定。
说明该路径在昇腾上是 **kernel-launch 受限**而非算力受限——每项约 10–15 次 kernel
启动，设备大部分时间在空转。把多项批量化可望进一步压低，是后续优化方向。

与 CPU 的交叉点在 **n≈18**（同机 numpy complex128 / torch complex64，单线程）：

| n | numpy c128 | torch c64 | 昇腾 | |
| --- | --- | --- | --- | --- |
| 14 | 1.46 ms | 1.90 ms | 17.87 ms | NPU 慢 12.3× |
| 16 | 9.99 ms | 6.01 ms | 20.58 ms | NPU 慢 3.4× |
| 18 | 71.22 ms | 32.24 ms | 24.12 ms | **NPU 快 1.34×** |
| 20 | 347.01 ms | 192.89 ms | 32.34 ms | **NPU 快 5.96×** |
| 22 | 1640 ms | 741.6 ms | — | — |

小比特数上 NPU 更慢是启动开销主导的必然结果，不必掩饰；论文应如实给出交叉点。
...
### 基准：补齐轴 C–G，接入 qiskit-aer，首份归档实测

`scripts/bench/axes.py` 补齐五轴：C（VQE 端到端）、D（峰值内存）、E（昇腾 vs CPU，
需真机）、F（多卡强/弱扩展，需真机）、G（能力矩阵）。装上 `qiskit-aer 0.17.2`，
高性能 CPU 基线首次进表。

首份归档证据：`docs/evidence/benchmarks/80618ce8…/cpu.json`
（`release_gate=PASS`、`worktree_dirty=false`、单线程、Accelerate BLAS）。

**GHZ 执行时间中位数（秒，complex128，单线程）：**

| n | aicir | cirq | qiskit | qiskit_aer |
| --- | --- | --- | --- | --- |
| 12 | 0.000420 | 0.000749 | 0.000347 | 0.000302 |
| 16 | 0.003565 | 0.002142 | 0.002523 | 0.003284 |
| 18 | 0.029179 | 0.005684 | 0.024794 | 0.013434 |
| 20 | 0.122435 | **0.022925** | 0.269016 | 0.069852 |

**诚实结论**：aicir 在 CPU 上处于中游。大 n 时优于 Qiskit 的参考 `Statevector`
路径（`n=20` 快 2.2×），但**落后 Aer 约 1.8×、落后 Cirq 约 5.3×**。Cirq 同为
numpy 实现却领先明显，说明差距不在语言而在实现策略（就地跨步更新、更少的中间
分配）——本轮的跨步快路径只走了这条路的一半：`_apply_local_strided` 仍分配整份
输出缓冲与两个中间量，峰值内存实测为理论下界的 4.0×（Cirq 5.0×，Aer 因 C++ 分配
不可见而显示 1.0×）。
...
- **`aicir.qml.diagnostics`：训练期贫瘠高原诊断（qml 成熟化路线第 7 项，收官）。** 把梯度方差信号 + QFIM 谱接到真实训练目标 `QFun`（区别于 `metrics.trainability` 的固定 local-probe）：`gradient_variance(qfun, n_params, ...)` 采随机参数算真实目标梯度的逐参数方差（贫瘠高原信号）；`barren_plateau_scan(make_qfun, n_qubits, ...)` 跨比特数扫描并拟合 log2(方差) 斜率判定（< -0.5 判 barren）；`qfim_spectrum(qfun, params)` 给 QFIM 特征谱与有效秩（近零特征值 = 平坦方向/过参数化）。梯度经 `QFun.grad`（`qml.deriv` 单一事实来源），QFIM 经 `qml.qfim`。同新增 `QFun.statevector(params)`（测量前末态振幅，供 QFIM 等以线路态为对象；噪声路径不支持）。已挂入 `qml` NPU 探针（`gradient_variance` case）。
- **`aicir.qml.QuantumKernel`：BatchSV 量子核（qml 成熟化路线第 5 项）。** 量子核 `K(x,z)=|<Φ(x)|Φ(z)>|²`：用 `BatchSV` 一次批量演化全部 N 个特征态（O(N)），再以实/虚分离矩阵乘算 gram（全实数 matmul，NPU 安全），替代逐对重演化 O(N²)（对比 `encoder.IQPEncoder.kernel_matrix`）。`matrix(X, Z=None)` 返回 numpy `(N, M)` 核矩阵（可喂 sklearn 预计算核 SVM），`__call__(x, z)` 单对。同附 `angle_feature_map` 便捷模板。特征映射为全数据驱动参数模板（门限 BatchSV 支持集）；非线性映射由调用方预算入角度矩阵。已挂入 `qml` NPU 探针（`quantum_kernel` case）。
- **生成元感知 `select_diff` + `QFun` auto 校正（qml 成熟化路线第 3 项）。** 新增 `aicir.qml.diff.circuit_shift_rule`：按线路参数门的 GateSpec `shift_rule` 归类 `two_term`/`four_term`/`mixed`。`select_diff` 加可选 `circuit=`：仅当优选落到两项 `psr` 时按门谱校正——全激发门（`single/double_excitation`，谱 {-1,0,1}）升级 `psr4`，两/四项混合降 `fd`（psr/psr4 均非逐参数正确）；`auto`（伴随 AD）/`fd` 不受影响，不传 `circuit` 时行为不变。`QFun` 的 `differential="auto"` 现传入线路，激发/uccsd 线路自动获正确 `psr4` 梯度（此前静默选 `psr` → 错）；且 torch 后端上 auto 落到 torch-AD 时退回参数移位家族（`QFun.grad` 为 float 接口无法驱动 AD，此前崩溃）。`qfun(device=...)` 现也接受后端实例（与 `BatchLayer`/`build_classifier` 一致）。
- **qml 梯度支持矩阵可执行契约 + NPU 层栈探针（qml 成熟化路线第 4 项）。** `tests/qml/test_support_matrix.py` 把「方法 × 后端族 × shots × 噪声 × 线路生成元谱 → select_diff 选择」固化为被测表（numpy/torch 两族逐单元断言 + 每方法可解析/算出有限梯度）。新增 `scripts/npu/qml_layers_probe.py`（QFun expval/probs 梯度、BatchLayer 前反向、build_classifier 一步训练均在 `NPUBackend` 实跑），并挂入 `qml` NPU suite（`scripts/npu/qml.sh`）——此前 `tests/qml` 在 strict-NPU 下仍走 cpu-torch，不触设备。
...
3. **MPS shape/长度读取 `to_numpy` 整个张量（已修）。**
   - 缺陷：`to_statevector` 逐 site 把整个收缩中间张量（趋近 2^n）、`_move_center_right/left` 每次 SVD 把奇异值向量下传 CPU，只为读维度——NPU 下每次都是强制同步 + 零信息增益的 D2H。
   - 修复：直接读张量 `.shape`（numpy/torch 通用），零传输。
4. **`State.reorder_endianness` 逐振幅 Python 循环（已修）。**
   - 缺陷：D2H 后对 2^n 个振幅逐个 Python 循环做位反转置换（n=22 本机约 1.3s），`msb()`/`lsb()` 大 n 不可用。
   - 修复：n 次移位构造位反转索引后一次向量化 gather；顺带删除孤立的 `_reverse_bits`。新增 n=22 性能哨兵与小 n 位反转定义对照测试。
5. **`Measure.run` 每 shot 重复上传初始态（已修）。**
   - 缺陷：噪声/in-circuit 分支每条轨迹重跑 `np.asarray` + `backend.cast`——`initial_density_matrix` 为 M 次相同 4^n H2D 上传（数组 `initial_state` 同理 2^n×M）。
   - 修复：`fresh_state` 的初始态归一化一次后全轨迹共享（轨迹执行不就地修改输入张量，局部门写入 `empty_like` 新缓冲；`State` 输入分支此前已是共享先例）。
6. **`NoiseModel` Kraus 算符每门每轨迹重建（已修）。**
   - 缺陷：`apply` 每个匹配门、每条轨迹都重跑 `kraus_operators`——嵌入函数用 2^n 次 Python 循环构建稠密 `(2^n,2^n)` numpy 再 `backend.cast`（每次全新 H2D），M shots × G 门 × k Kraus 地重复构建从不变化的算符，是噪声路径的主要浪费。
   - 修复：按 `(规则下标, n_qubits, id(backend))` 缓存全系统嵌入的 `(K, K†)` 对；信道加入规则后不应再就地修改参数（docstring 注明）。`_embed_operator` 的 Python 循环本身未重写（缓存后只跑一次）。
7. **密度分支为取对角线下传整个 ρ（已修）。**
   - 缺陷：`_born_probs`/`joint_parity_probs` 密度分支 `to_numpy` 整个 `(2^n,2^n)` 密度矩阵只为取 `np.diag`。
   - 修复：改经 `State.probabilities` 的设备侧 `.diagonal()` 路径，传输量 4^n→2^n。
8. **密度投影 `np.outer(keep, keep)` 物化 4^n 布尔掩码（已修）。**
   - 缺陷：`_project_subset_outcome` / `_project_parity_rotated` 密度分支构造全尺寸 `(2^n,2^n)` bool 掩码 + `np.where` 整拷贝——n=16 时约 4 GB 掩码，纯多余分配。
   - 修复：行列置零 `rho[~keep,:]=0; rho[:,~keep]=0`，零额外矩阵分配。
9. **`projector._reset_dm` O(4^n) Python 双循环（已修）。**
   - 缺陷：逐 (r,c) 元素嵌套 Python 迭代，n=13 本机约 3s，密度矩阵 reset 超过 ~13 比特不可用。
   - 修复：整形为 (L,2,R,L,2,R) 秩-6 张量后对目标比特 0/0、1/1 对角子块求和；顺带删除孤立的 `_replace_bit`。新增与 Kraus 信道定义 K0ρK0†+K1ρK1† 的逐元素等价测试与 n=13 性能哨兵。
10. **密度/噪声路径全尺寸门矩阵每轨迹重建（已修）。**
    - 缺陷：门对象与矩阵在 M 条轨迹间完全相同，但 `_apply_unitary` 每条轨迹对每个门重跑 `gate_to_matrix(cir_qubits=n)` 的 kron 链——M×G 次 4^n 构建/分配，是 Kraus 缓存落地后噪声路径的剩余主项。
    - 修复：`Measure.run` 为每次运行建一个按 `id(gate)` 键控的矩阵缓存，经 `run_trajectory`/`_exec_ops` 传入 `_apply_unitary`；torch 参数带 `requires_grad` 时绕过缓存（不改变 autograd 图形状，NPU fan-out 梯度累加不可用）。
11. **`Measure._build_result` observables 在 CPU 计算（已修）。**
    - 缺陷：把整个态（2^n 向量或 4^n 密度矩阵）`to_numpy` 下传后与每个算符做稠密 numpy matmul。
    - 修复：改走 `State.expectation` → `backend.expectation_sv/expectation_dm`（NPU 安全实现已存在），每个 observable 只传一个标量。
