...
def circuit_to_python_source(
    ...
    plot_cmd: str = "python -m demos.H2O.H2O_cir",
    optimize: bool = False,
) -> str:
    ...
    diagram next to itself. ``generated_by``/``description``/``regen_cmd``/
    ``plot_cmd`` only customise the generated module's header comment. When
    ``optimize=True`` the generated ``__main__`` runs the circuit through
    :func:`aicir.transpile.optimize` and plots the optimised circuit.
    """
    ...
    if optimize:
        lines.append("    optimized = optimize(circuit)")
        lines.append(
            '    print(f"Optimized circuit: {len(circuit.gates)} -> {len(optimized.gates)} gates")'
        )
        lines.append(f"    plot(optimized, figure_path, title={title!r})")
    else:
    ...
    if optimize:
        plot_idx = lines.index("from aicir.visual import plot")
        lines.insert(plot_idx + 1, "from aicir.transpile import optimize")
    return "\n".join(lines)


def save_circuit_python(circuit, file_path, func_name: str = "build_h2o_qas_circuit", **source_kwargs) -> None:
    """Write ``circuit`` to ``file_path`` as an importable Python module.

    Extra ``source_kwargs`` (``figure_name``/``title``/``generated_by``/
    ``description``/``regen_cmd``/``plot_cmd``) pass through to
    :func:`circuit_to_python_source`.
    """

    Path(file_path).write_text(
        circuit_to_python_source(circuit, func_name, **source_kwargs), encoding="utf-8"
    )
