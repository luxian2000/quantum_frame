...
本次结果使用 `demos/BeH2/BeH2.py` 中的 16 比特 BeH2 活性空间 Hamiltonian（3-21G 基组、6 电子/8 空间轨道活性空间、Jordan-Wigner 映射，共 1313 个 Pauli 项），以及 `demos/BeH2/BeH2_cir_spin.py` 中保存的、在 **8 块 Ascend NPU**（`device=npu:0`，`world_size=8`，搜索内分片 in-search sharding）上由 supernet QAS 搜索得到的线路。
...
| 项目 | 能量 / Ha |
| --- | ---: |
| Hamiltonian 精确基态能量 | -19.1421241760 |
| 线路能量 - 精确基态能量 | +0.0024450283 |
| 绝对误差 | 0.0024450283 |
| NPU 搜索报告 fine-tuned energy | -19.1396827698 |
| 固定 ansatz VQE 基线 | -19.1405792236 |
| 保存线路重新计算能量 | -19.1396791478 |
| 保存线路能量 - NPU 报告能量 | +0.0000036220 |

本次精确基态能量由对角化求得（`eigsh(Lanczos,SA)@cpu`）：构造 16 比特 Hamiltonian（2^16=65536 维）后取最小特征值。

- 保存线路能量比精确基态能量高约 `2.445 mHa`，处于化学精度（约 1.6 mHa）之外（线路能量 ≥ 精确基态能量，满足变分下界）。
- 同一次运行的固定 ansatz VQE 基线 `-19.1405792236 Ha` 相对精确基态高约 `1.545 mHa`。

结论：

- 保存线路（`BeH2_cir_spin.py`）经 statevector 复算得到 `-19.1396791478 Ha`，与 NPU 运行报告的 `fine-tuned energy = -19.1396827698 Ha` 一致，相差约 `3.62e-06 Ha`，源于 NPU complex64 精度与求值路径差异。
- 值得注意的是，同一次 NPU 运行中**固定 ansatz 的 VQE 基线 `-19.1405792236 Ha` 比 QAS 搜索到的线路更低（更优）约 `0.90 mHa`**。也就是说，在这组 16 比特 / `L=4` 配置下，supernet 搜索本身没有收敛到优于固定基线的结构——这与 H2O 的 NPU 版（`demos/H2O/result_npu.md`）观察到的现象一致：更大的搜索空间加剧了权重共享 supernet 的“竞争”，最终选出的子线路欠优化。若要改善，可考虑增大 `supernet_steps`/`finetune_steps`、调整 `supernet_num (W)` 或减小层数。

## 搜索配置

本次 NPU 运行（见 `demos/BeH2/output_spin.txt` 头部，对应 `demos/BeH2/BeH2_npu.py`）的设置：

- 设备：8 块 Ascend NPU，`device=npu:0`，`world_size=8`，`mode=safe`（与单卡数值等价的搜索内分片）
- 量子比特数：16，层数（深度）：4
...
- 任务 `task="vqe"`
- 总墙钟时间：约 14249.4 s（约 3.96 小时）

本次保存线路包含 97 个门：

- 6 个 `pauli_x` 门用于制备 HF 参考态
- 57 个 `single_excitation` 门
- 34 个 `double_excitation` 门

（NPU 报告中的 `excitations = 91 (single=57, double=34)` 仅统计激发门，不含 6 个 HF 制备门。）
...
```text
exact   = -19.1421241760 Ha
circuit = -19.1396791478 Ha
```

## 复现方式

```bash
# 在 8 块 Ascend NPU 上重新搜索（需 torch_npu，约 4 小时）
torchrun --nproc_per_node=8 demos/BeH2/BeH2_npu.py --gradient psr --output output_spin.txt
