...
# 2. 线路能量复算
# ---------------------------------------------------------------------------
def compute_circuit_energy(backend) -> float:
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.skip_exact:
        ...
        exact_energy, exact_path = exact_ground_energy(args.method, device)
        print(f"exact   : {exact_energy:.10f} Ha   [{exact_path}, {time.time() - t0:.1f}s]")

    circuit_energy = compute_circuit_energy(backend)
    ...
    markdown = render_markdown(circuit_energy, exact_energy, exact_path, device, report)
