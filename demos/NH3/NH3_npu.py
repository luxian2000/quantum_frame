"""在 4 张 Ascend NPU 上用 supernet QAS 搜索 NH3 的 VQE 线路（内搜索分片）。

方法选择：与 ``demos/NH3/NH3.py`` 一致，使用 ``supernet`` 方法，但通过统一入口
...
PPR-DQL / PPO-RB 面向小规模目标态制备，不适合 12 量子比特分子哈密顿量。
...
12 比特 NH3 Hamiltonian 含 923 个 Pauli 项。本版本默认使用 autograd 梯度
...
closed-shell Hartree-Fock 参考态（占据比特见 ``nh3_vqe_qas_kwargs``），再从
...
- ``NH3_npu_cir.qasm``    : 全局最优线路（QASM；激发门暂不支持时跳过）
- ``NH3_cir_spin.py``     : 全局最优线路（Python）
- ``NH3_cir_spin.png``    : 全局最优线路图
- ``result_spin.md``      : 分析文件（精确基态能量 + 复算线路能量对比）

分析文件 ``result_spin.md`` 由本脚本在 rank 0 搜索结束后内联生成（复用
``demos/NH3/NH3_result_spin.py`` 的 ``generate_result_md``）；也可单独运行
``python -m demos.NH3.NH3_result_spin`` 解析 ``output_spin.txt`` 重新生成。
注意 12 比特精确对角化在内存不足时会自动退回 ``--skip-exact`` 仅复算线路能量。

运行方式（4 卡）：

    torchrun --nproc_per_node=4 demos/NH3/NH3_npu.py [--mode safe|aggressive]

无 NPU 的本地冒烟测试（单进程、允许 CPU 回退，分片路径未激活）：

    python demos/NH3/NH3_npu.py --allow-cpu-fallback
"""
...
from demos.NH3.NH3 import (
    NH3_BASIS,
    NH3_CHARGE,
    NH3_QUBIT_MAPPER,
    NH3_SPIN,
    nh3_vqe_qas_kwargs,
    build_nh3_hamiltonian,
    save_circuit_python,
)
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="NH3 12-qubit supernet VQE QAS on 4 Ascend NPU (task-parallel).",
    )
...
def _write_report(
    ...
    *,
    ...
    lines = [
        "=" * 88,
        "NH3 supernet VQE QAS on Ascend NPU (in-search sharding, single run)",
        "=" * 88,
        ...
        f"basis/charge/spin/map : {NH3_BASIS}/{NH3_CHARGE}/{NH3_SPIN}/{NH3_QUBIT_MAPPER}",
        ...
        "  circuit files      : NH3_npu_cir.qasm, NH3_cir_spin.py, NH3_cir_spin.png",
        "",
    ]
...
def main(argv: list[str] | None = None) -> None:
    ...
    _log("=== NH3 supernet VQE QAS on Ascend NPU ===", rank)
    ...
    if rank == 0:
        print(
            f"NH3 active-space Hamiltonian: basis={NH3_BASIS} charge={NH3_CHARGE} "
            f"spin={NH3_SPIN} mapper={NH3_QUBIT_MAPPER}"
        )
    ...
    if rank == 0:
        ...
        qasm_path = out_dir / "NH3_npu_cir.qasm"
        ...
        py_path = out_dir / "NH3_cir_spin.py"
        save_circuit_python(
            result.best_circuit,
            py_path,
            func_name="build_nh3_npu_qas_circuit",
            figure_name="NH3_cir_spin.png",
            title="NH3 supernet ground-state ansatz (spin-preserving)",
        )
        print(f"Python circuit saved to: {py_path}")

        png_path = out_dir / "NH3_cir_spin.png"
        try:
            ...
            plot(
                result.best_circuit,
                png_path,
                title="NH3 supernet ground-state ansatz (spin-preserving, L=%d)" % args.layers,
            )
        ...
        try:
            from demos.NH3.NH3_result_spin import generate_result_md

            try:
                md_path, _, _ = generate_result_md(
                    result.best_circuit, result_report, backend=backend, device=device
                )
            except Exception as exc:  # 12 比特精确对角化很重，内存不足时退回仅复算线路能量
