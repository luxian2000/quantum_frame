"""求 NH3 精确基态能量、复算线路能量并生成 result_spin.md（在 NPU 机器上运行）。

    python -m demos.NH3.NH3_result_spin                 # 默认稀疏 Lanczos，几分钟
    python -m demos.NH3.NH3_result_spin --method dense  # 全谱稠密对角化（大内存机器）
    python -m demos.NH3.NH3_result_spin --allow-cpu-fallback  # 本地冒烟测试

流程：
1. 精确基态能量：用“带符号置换”逻辑高效构造 12 比特 Hamiltonian（每个 Pauli 串每行只有一个
   非零，逐项向量化累加，避免 to_matrix 的逐项 2^12×2^12 临时矩阵），再取最小特征值。
   - 默认 ``--method sparse``：scipy Lanczos ``eigsh(k=1, which="SA")``，只求最小特征值；
     12 比特 / 923 项的 CSR 构造较 BeH2 轻（量子比特更少），普通大内存机器即可；
   - ``--method dense``：完整稠密对角化。注意 Ascend 无 complex ``eigvalsh`` 内核，torch 会
     自动回退到 CPU 跑全谱（2^12 维稠密极大、很慢），仅建议在超大内存机器上使用。
2. 逐门把保存线路作用到态向量（12 比特无法构造 2^12 全局 unitary，故不走 circuit.unitary()），
   再对全部 Pauli 项逐项计算 <psi|P|psi> 求和得到线路能量；
3. 解析 output_spin.txt 中 NPU 运行报告的能量/配置；
4. 写出 demos/NH3/result_spin.md。
"""
...
from demos.NH3.NH3 import nh3_vqe_qas_kwargs, build_nh3_hamiltonian
from demos.NH3.NH3_cir_spin import build_nh3_npu_qas_circuit
...
def exact_ground_energy(method: str, device: str, dtype=np.complex64) -> tuple[float, str]:
    ...
    method="sparse"（默认，推荐）：scipy Lanczos `eigsh(k=1, which='SA')`，只求最小特征值；
        12 比特 / 923 项的 CSR 构造较 BeH2 轻（量子比特更少），普通机器即可完成。
    method="dense"：完整稠密对角化。注意 Ascend 无 complex `eigvalsh` 内核，torch 会自动
        回退到 CPU 跑全谱（2^12 维稠密较大、稍慢），故仅在超大内存机器上使用。
    """
    if method == "sparse":
...
def compute_circuit_energy(backend, circuit=None) -> float:
    ...
    ``circuit`` 为 None 时读取 ``NH3_cir_spin.py`` 保存的线路；也可直接传入
    搜索返回的 ``result.best_circuit`` 以避免依赖落盘文件（供 NH3_npu.py 内联调用）。
    """
    if circuit is None:
...
def render_markdown(
    ...
    n_qubits = report.get("n_qubits", 12)
    dim = 1 << n_qubits
    n_terms = report.get("n_terms", 923)
...
- 同一次运行的固定 ansatz VQE 基线 `{base:.10f} Ha` 相对精确基态高约 `{base_err_mha:.3f} mHa`。"""
    ...
    return f"""# NH3 基态能量结果（NPU 自旋保持 ansatz 版）

本次结果使用 `demos/NH3/NH3.py` 中的 {n_qubits} 比特 NH3 活性空间 Hamiltonian（STO-3G 基组、6 电子/6 空间轨道活性空间、Jordan-Wigner 映射，共 {n_terms} 个 Pauli 项），以及 `demos/NH3/NH3_cir_spin.py` 中保存的、在 **{ws} 块 Ascend NPU**（`device={report.get('device', 'npu:0')}`，`world_size={ws}`，搜索内分片 in-search sharding）上由 supernet QAS 搜索得到的线路。
...
- 保存线路（`NH3_cir_spin.py`）经 statevector 复算得到 `{circuit_energy:.10f} Ha`，与 NPU 运行报告的 `fine-tuned energy = {ft:.10f} Ha` 一致，相差约 `{abs(delta_report):.2e} Ha`，源于 NPU complex64 精度与求值路径差异。
...
本次 NPU 运行（见 `demos/NH3/output_spin.txt` 头部，对应 `demos/NH3/NH3_npu.py`）的设置：
...
```text
demos/NH3/NH3_cir_spin.py
demos/NH3/NH3_cir_spin.png
demos/NH3/output_spin.txt
demos/NH3/NH3_npu_cir.qasm
```
...
1. **精确基态能量**：用同一套“带符号置换”逻辑高效构造 {n_qubits} 比特 Hamiltonian（每个 Pauli 串每行只有一个非零，逐项向量化累加，避免 `to_matrix` 的逐项 2^{n_qubits} 临时矩阵），再取最小特征值。默认走稀疏 Lanczos `scipy.sparse.linalg.eigsh(k=1, which="SA")`；12 比特 / {n_terms} 项的 CSR 构造较 BeH2 轻（量子比特更少）。Ascend 当前没有 complex `eigvalsh` 内核，故 `--method dense` 的全谱稠密对角化会被 torch 自动回退到 CPU 运行（2^{n_qubits} 维稠密较大、稍慢），仅建议在超大内存机器上使用。
...
python -m demos.NH3.NH3_result_spin

# 如需完整稠密对角化（超大内存机器；Ascend 无内核会回退到 CPU 全谱）
python -m demos.NH3.NH3_result_spin --method dense
```
...
torchrun --nproc_per_node={ws} demos/NH3/NH3_npu.py --gradient {gradient} --output output_spin.txt

# 仅根据已记录的线路重新绘图
python -m demos.NH3.NH3_cir_spin
```
"""
...
def generate_result_md(
    ...
    *,
    ...
    供 ``NH3_npu.py`` 在搜索结束后内联调用：直接传入 ``result.best_circuit`` 与内存中的
    ``report`` 字典，无需再解析 ``output_spin.txt``。``skip_exact=True`` 跳过（12 比特下很重的）
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="NPU 稀疏/稠密对角化求精确基态能量 + 复算 NH3 线路能量并生成 result_spin.md。",
    )
    parser.add_argument(
        "--allow-cpu-fallback",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="精确基态求解法：sparse=Lanczos eigsh（默认，COO 中间体可达数十 GB）；"
             "dense=全谱稠密对角化（2^12 维，Ascend 无内核会回退 CPU）。",
    )
