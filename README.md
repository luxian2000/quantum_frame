...
> **训练（autograd backward）**：`complex64` 的**反向**内核同样缺失——朴素实现中，复数张量的梯度累加会触发 `aclnnAdd ... DT_COMPLEX64`，导致 `loss.backward()` 直接报错。`NPUBackend` 通过自定义 `torch.autograd.Function`（`matmul` 与 `expectation_sv`，见 `_NpuMatmulFn` / `_NpuExpectationFn`）解决了这一点：实/虚部拆分只发生在 Function 的 forward/backward **内部**（不进入 autograd 计算图），因此计算图里只剩“线性使用”的复数节点，反向时不会发生复数梯度累加。其梯度与原生复数 autograd 数值一致（已在 CPU 上对齐验证），所以**在 NPU 上可以直接用 `loss.backward()` 训练**，QAS supernet 也因此走标准 autograd 路径（比参数移位快）。
>
