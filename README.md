...
> **训练（autograd backward）限制**：上述兼容路径解决的是**前向**计算；`complex64` 的**反向**同样缺失内核（梯度累加会触发 `aclnnAdd ... DT_COMPLEX64`）。因此在 NPU 上做变分训练时不能直接用 `loss.backward()`。QAS supernet 已对此自动处理：当运行设备为 NPU 时自动切换到**参数移位规则**（parameter-shift，仅前向计算；对 `rx/ry/rz/rzz` 这类 Pauli 旋转门是精确梯度），无需手动设置；其代价是每个参数每步需要 2 次前向求值，比 GPU autograd 慢。若自行在 NPU 上写训练循环，请同样避免对复数张量做 backward，改用参数移位或在 CPU 上做反向。
