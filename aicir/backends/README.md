...
11. [硬件能力探测（npu_probe，§5.3）](#53--硬件能力探测npu_probe)
...
> 各 rank 领取**批次项（整条电路 / 候选）** 的子集，每个 NPU 独立运行分到的**整条电路**——
> 单条电路的门矩阵 `2^n×2^n` 与态向量 `2^n` 始终完整驻留在一个 NPU 上，不跨设备拆分；
> 结束后归并（见下文「分布式任务分发」）。
...
| **NPU 安全** | 可在 Ascend NPU 上直接演化与反向训练 |
