...
- `core/`：传统 QAS 的类型、评估器、reward、search env、统一 runner/config、`SearchStrategy`（`strategy.py`）/策略注册表（`registry.py`）、内置策略适配（`strategies.py`）、backend 解析（`backend_utils`）与 NPU 分片原语（`sharding`）。
