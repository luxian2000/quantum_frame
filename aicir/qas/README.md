...
- `core/`：传统 QAS 的类型、评估器、reward、search env、统一 runner/config、`SearchStrategy` 协议与策略注册表（`registry.py`）、内置策略适配（`strategies.py`）、backend 解析（`backend_utils`）与 NPU 分片原语（`sharding`）。
- `algorithms/`：`supernet.py`、`mogvqe.py`、`pporb.py`、`pprdql.py`、`crlqas.py`、`qdrats.py`、`dqas.py` 等具体算法实现。
...
- 多目标遗传 VQE 拓扑搜索：`mogvqe.py` 将线路表示为二量子比特 block 序列，通过 NSGA-II 同时最小化能量和 CNOT 数量，适合从 block-based HEA 出发压缩 VQE ansatz。
...
- 强化学习线路搜索：`pporb.py`、`pprdql.py` 和 `crlqas.py` 从空线路出发逐步追加 aicir 支持的量子门，搜索目标态制备或低能量线路。
...
方法名大小写不敏感，也支持常见别名，例如 `"h2"`/`"h2_vqe"`（→ `supernet_h2`）、`"ppr"`/`"ppr_dql"`（→ `pprdql`）、`"crl"`（→ `crlqas`）、`"ppo"`/`"ppo_rb"`（→ `pporb`）、`"differentiable_qas"`（→ `dqas`）。
...
- `config.pporb(...)`：PPO-RB 超参数配置（`config.ppo_rb(...)` 作为旧名别名保留）
- `config.pprdql(...)`：PPR-DQL 超参数配置（`config.ppr_dql(...)` 作为旧名别名保留）
...
- `block_hardware_efficient_ansatz(n_qubits, layers=1, topology="linear")` / `mogvqe(initial_ansatz, hamiltonian=None, energy_evaluator=None, config=None, backend=None)`
...
当前实现使用 `aicir.core.Circuit`、aicir 门构造器、`GPUBackend` 和 aicir 态向量/矩阵演化能力，不依赖 PennyLane、Qiskit、Cirq 或其他量子 SDK。VQE/H2 内置目标支持 `noise_mode="depolarizing"` 或 `"amplitude_damping"` 的密度矩阵评估；启用噪声后训练/微调会强制走参数位移更新，避免把噪声 Kraus 通道纳入复数 autograd。

Supernet assignment 规则作为默认实现：每个采样 ansatz 会先在全部 `W=supernet_num` 个 supernet 上计算 objective，然后分配给 loss 最小的 supernet，并只更新该 supernet 的活跃参数。排序阶段默认随机采样；也可用 `ranking_strategy="evolutionary"` 启用突变 + 截断选择。
...
| `n_qubits`                  |                        `3` | 量子比特数。                                                                         |
| `layers`                    |                        `3` | ansatz 层数。                                                                        |
| `single_qubit_gates`        | `("i", "h", "rx", "ry", "rz")` | 单量子比特候选门池，支持 `i`、`h`、`rx`、`ry`、`rz`。               |
| `two_qubit_gates`           |            `("cx", "rzz")` | 双量子比特候选门池（`rzz` 为带参纠缠门）。                                          |
| `two_qubit_pairs`           | `((0, 1), (0, 2), (1, 2))` | 允许搜索的连接对，格式为 `(control, target)`。                                    |
...
| `ranking_strategy`          |                 `"random"` | 排序阶段采样策略；可选 `"random"` 或 `"evolutionary"`。                          |
| `use_evolutionary_ranking`  |                    `False` | 兼容开关；为 `True` 时等价于 `ranking_strategy="evolutionary"`。                 |
| `ranking_generations`       |                        `4` | evolutionary ranking 的突变/选择轮数。                                            |
| `ranking_mutation_rate`     |                      `0.2` | 每个架构槽位的突变概率。                                                          |
| `noise_mode`                |                   `"none"` | VQE/H2 目标的噪声模式：`"none"`、`"depolarizing"` 或 `"amplitude_damping"`。    |
| `noise_config`              |                     `None` | noisy QAS 配置（`NoiseConfig(enabled, probability, channel, after_gates)`）。    |
| `shard_mode`                |                   `"safe"` | 多 NPU 单次搜索分片模式（`"safe"`/`"aggressive"`，见 §3.8）。                    |
| `four_qubit_gates`          |                       `()` | 四量子比特候选门池（默认空，面向化学激发门等扩展）。                                 |
| `four_qubit_groups`         |                       `()` | 四量子比特门作用的比特组。                                                           |
| `search_four_qubit_gates`   |                     `True` | 是否搜索四量子比特门布局（仅在 `four_qubit_gates` 非空时生效）。                   |
| `hf_occupied_qubits`        |                       `()` | Hartree-Fock 初占据比特（用于初态制备）。                                            |

实践建议：

- 分类任务默认搜索空间为 3 量子比特、3 层，并在 `(0, 1)`、`(0, 2)`、`(1, 2)` 上搜索 CNOT/无 CNOT。代码默认单量子比特门池为 `(i, h, rx, ry, rz)`；若按论文复现实验固定为单一 `RY`（`single_qubit_gates=("ry",)`），则 `layers=3` 时逻辑搜索空间大小为 `8^3`（仅 CNOT mask）。
...
## 4. MoG-VQE（`mogvqe`）：基于 NSGA-II 的多目标遗传 VQE 拓扑搜索

`algorithms/mogvqe.py` 实现 MoG-VQE 的线路拓扑搜索部分。输入是 block-based hardware-efficient ansatz，算法把线路表示为二量子比特 block 的有序列表，通过插入 block、删除 block 和大尺度变异修改拓扑，并使用 NSGA-II 同时最小化能量和 CNOT 数量。输出是修改后的 aicir `Circuit`、最终 Pareto 前沿和每一代搜索摘要。
...
- `mogvqe(initial_ansatz, hamiltonian=None, energy_evaluator=None, config=None, backend=None)`
...
`mogvqe` 至少需要 `hamiltonian` 或 `energy_evaluator` 之一。返回值为 `MOGVQEResult`：
...
```python
from aicir.qas import MOGVQEConfig, block_hardware_efficient_ansatz, mogvqe
...
result = mogvqe(initial, energy_evaluator=energy_evaluator, config=cfg)
...
result = mogvqe(initial, hamiltonian=ham, config=cfg)

print(result.best_energy)
print(result.best_circuit.show())
```

## 5. PPO-RB（方法名 `pporb`）：基于真正近端策略优化的量子架构搜索

统一入口方法名为 `pporb`，实现文件为 `algorithms/pporb.py`。旧方法名 `ppo_rb` 仍作为兼容别名保留。PPO-RB 的输入是目标密度矩阵，输出是策略参数 `theta` 与搜索得到的 `Circuit`。
...
## 6. PPR-DQL（方法名 `pprdql`）：基于持续强化学习和策略复用的量子架构搜索

统一入口方法名为 `pprdql`，实现文件为 `algorithms/pprdql.py`。旧方法名 `ppr_dql` 仍作为兼容别名保留。PPR-DQL 的输入是目标 `State`，可直接返回 `Circuit`，也可以返回包含训练信息的结果对象。
...
| `architecture_reward_evaluator` | `None` | 可选架构奖励评估器；为奖励叠加结构项（默认关闭）。                                |
| `architecture_reward_weight`    | `0.0`  | 架构奖励项权重。                                                                  |
...
`config.pprdql(action_gates=...)` 支持自定义动作门集合。每个动作是一个门字典，格式与 `Circuit` 门定义一致。
...
cfg = config.pprdql(action_gates=custom_actions)
result = run("pprdql", target_state=state, config=cfg)
...
## 7. CRLQAS（方法名 `crlqas`）：面向硬件误差的课程强化学习量子架构搜索

统一入口方法名为 `crlqas`，实现文件为 `algorithms/crlqas.py`。CRLQAS 的目标是最小化给定哈密顿量的能量。结构搜索由 DDQN 决策，参数优化由 Adam-SPSA 执行。
...
- `PPO_RB_demo_ghz4.py` / `PPO_RB_demo_dicke3.py` / `PPO_RB_demo_w3.py`：使用 PPO-RB 搜索 GHZ4 / Dicke3 / W3 线路
- `PPR_DQL_demo_ghz3.py`：使用 PPR-DQL 搜索 3 比特 GHZ 线路，并导出 OpenQASM 3.0 到 `demos/ppr_dql_ghz3_circuit.qasm`
- `PPR_DQL_noise_adaptive_demo.py`：PPR-DQL 的噪声自适应搜索示例
- `CRLQAS_demo_h2.py`：使用 CRLQAS 搜索小分子 H2 的低能量线路，并导出 OpenQASM 3.0 到 `demos/crlqas_h2_circuit.qasm`
- `MoG_VQE_demo_h2.py`：使用 MoG-VQE 对 H2 做多目标遗传拓扑搜索
- `VQA_QAS_demo_hamiltonian_path.py` / `VQA_QAS_demo_hamiltonian_cycle.py`：supernet VQA-QAS 在哈密顿量路径/环问题上的示例
- `architecture_scoring_demo.py`：架构评分指标（`ArchitectureEvaluator`）示例
