...
class LayerwiseAnsatzGene:
    ...
    def __post_init__(self) -> None:
        ...
        if self.n_qubits < 2:
            raise ValueError("LayerwiseAnsatzGene requires at least two qubits")
...
def sample_layerwise_genes(
    ...
    if edge_count <= 0:
        raise ValueError("layerwise gene sampling requires at least one entangling edge")
    ...
    # Chemistry-friendly deterministic seeds: mix X/Z rotations and Rxx/Rzz
    # entanglers so H2-like X/Y/Z Pauli structure is represented early.
    seed_patterns = [
...
def evaluate_vqe_energy(
    ...
    state = backend.cast(result.state)
...
def optimize_vqe_energy(
    ...
    if initial_parameters is not None:
        ...
        mode = "warm_start_then_random" if start_count > 1 else "warm_start"
    elif mode in {THETA_INIT_ZERO_DIAGNOSTIC, "zeros"}:
