...
This script is a scheduler wrapper around ``vqe_qas_run_fair_labels.py``.  It
does not change the VQE/oracle protocol: each shard is labeled by the existing
runner, then shard outputs are concatenated back into one benchmark table.
"""
...
from aicir.qas.vqe_qas_protocol import BENCHMARK_TABLE_FIELDS
...
def _runner_command(args: argparse.Namespace, shard_queue: Path, shard_output: Path, start_index: int) -> list[str]:
    runner = Path(__file__).with_name("vqe_qas_run_fair_labels.py")
    command = [
        str(args.python),
        str(runner),
        "--queue",
