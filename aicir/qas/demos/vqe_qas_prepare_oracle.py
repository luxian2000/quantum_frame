...
This script does not run VQE.  It builds the candidate metadata table, coverage
report, initial fair-label queue, and empty benchmark-table stub needed before
training an offline oracle.
"""
...
def _entanglement_score(candidate: CandidateRecord) -> float:
    n_qubits = max(2, int(candidate.metadata.get("n_qubits", 2)))
    layers = max(1, int(candidate.metadata.get("layers", 1)))
    max_edges = (n_qubits if candidate.topology == "ring" else n_qubits - 1) * layers
    return min(1.0, float(candidate.two_q_count) / max(1.0, float(max_edges)))


def _structural_expressibility_score(candidate: CandidateRecord) -> float:
    n_qubits = max(1, int(candidate.metadata.get("n_qubits", 1)))
    layers = max(1, int(candidate.metadata.get("layers", 1)))
    ...
    rotation_richness = {
        ...
        "ry_rz": 2.0 / 3.0,
        "rx_ry_rz": 1.0,
    }.get(rotation_block, 0.0)
    final_bonus = 0.15 if final_rotation == "ry_rz" else 0.05
    param_density = min(1.0, float(candidate.n_params) / max(1.0, float(3 * n_qubits * (layers + 1))))
    return float(max(0.0, min(1.0, 0.45 * rotation_richness + 0.35 * param_density + 0.20 * _entanglement_score(candidate) + final_bonus)))
    ...
    *,
    ...
    if structural_mode:
        ...
        for candidate, architecture in zip(candidates, architectures):
            expressibility = _structural_expressibility_score(candidate)
...
def main() -> None:
    ...
    parser.add_argument("--oracle-protocol-version", default="vqe_qas_trust_region_oracle_v1")
    parser.add_argument("--batch-id", default="initial_v1")
