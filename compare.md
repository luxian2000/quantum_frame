...
本文基于对两套本地代码库的通读，对 **aicir**（本仓库 `quantum_frame`）与 **WuYueSDK / 五岳**（`/Users/luxian/GitSpace/WuYueSDK`，本地版本 `wuyue==1.0`）两款量子电路模拟与量子算法框架做全面的功能梳理与横向对比。

> 更新口径：统计与功能判断基于 2026-07-04 的本地源码；未把第三方线上文档或不可见服务端能力计入，除非本地代码/文档有对应接口。
>
...
| 主打差异 | 后端可插拔（含 **Ascend NPU**）、QAS、丰富 metrics/transpile、多算法编排、**NPU 上可微分的张量网络/单/部分振幅模拟（cotengra 路径+切片）** | 全/密度矩阵/部分/单振幅/张量网络多模拟方法、**真实量子云 QPU** |
| 文档语言 | 中文为主 | 中文为主 |

一句话总结：

- **aicir** 是一个"宽而自洽"的研究型框架 —— 后端抽象干净、变分算法/架构搜索/度量/编译/编码器等子系统齐备，且能落到 **昇腾 NPU** 硬件加速。
- **WuYueSDK** 是一个"深在模拟引擎与工程化"的工业级 SDK —— 模拟方法多样（尤其张量网络与单/部分振幅）、经典寄存器与 if/while 控制流工程完整度高、并接入**真实量子计算云**，`qutorch` 提供成熟的 PyTorch 自动微分栈。
...
| 指标 | aicir | WuYueSDK |
| --- | --- | --- |
| Python 源文件数 | 201（`aicir/`） | 111（`wuyue/`） |
| 代码行数（包内 `.py`，不含测试） | ~44,766 | ~27,867 |
| 测试文件数 | 160（`tests/`，`pytest`） | 4（`test/`，`pytest`） |
...
**观察**：aicir 的测试覆盖面（160 个测试文件）与"零硬依赖"设计明显更克制、更利于集成；WuYueSDK 依赖较重（直接绑定 qiskit==1.4.3 / pennylane==0.43.0 / cirq==1.6.1），换来的是开箱即用的第三方互操作与云端能力。
...
| 能力 | aicir | WuYueSDK |
| --- | --- | --- |
| 态矢量模拟 | ✅ 统一 `State` 的向量形态 + `tn_statevector` | ✅ 全振幅 `Sim_full` |
| 密度矩阵/含噪模拟 | ✅ 统一 `State` 的矩阵形态 + `NoiseModel` | ✅ `Sim_dm` |
...
| 经典寄存器 | ✅ `ClassicalRegister`/`Bit`（`aicir.core.classical`） | ✅ `ClassicalRegister` / `Bit` |
| 经典控制流 if/while | ✅ `if_`/`while_`（`aicir.core.circuit`，走 `Measure.run` 测量轨迹路径） | ✅ `qif/trueif/falseif/qwhile`（`QuantumProg`） |
...
| OpenQASM I/O | ✅ QASM 2.0 + 3.0 + JSON 往返 | ⚠️ QASM2 导入/导出（部分门与参数形态受限） |
...
**结论**：两者都具备张量网络/单/部分振幅模拟且都可用 cotengra 规划路径。aicir 的独有差异：切片执行仍走后端抽象，因此**在昇腾 NPU 上可跑且保持可微**，可直接喂给 VQE/QAOA 等变分训练循环；WuYue 的独有差异在真实 QPU 云。
...
| 方面 | aicir | WuYueSDK |
| --- | --- | --- |
| 核心类 | `Circuit`（接受 `Operation`/`Measurement`/`ControlFlow`/旧 dict，内部规范化保存）；`Operation` 类型化 IR | `QuantumCircuit`（gate 对象）；`QuantumProg`（支持控制流） |
| 门表示 | 工厂函数返回不可变 `Operation`/`Measurement`/`ControlFlow`（带旧 dict 只读兼容与 `==`）；`Circuit.gates` 返回规范化的**类型化指令**，旧 dict 经 `legacy_gates`/`to_gate_dicts()` 获取 | **门对象**（`Basicgate` 及子类，如 `RX/CU3/RXX`） |
...
| 控制流 | `if_`/`while_`（测量反馈，`Measure.run` 逐 shot 轨迹执行；`while_` 必须给 `max_iterations`） | **`qif/trueif/falseif` + `qwhile`**（依赖测量结果的经典条件） |
| 逆电路 | 有 | `dagger()` |
| 批量 | `core/batch.py` | `set_batch/get_batch_size` |
| 深度/矩阵 | `unitary()` | `depth()`/`get_matrix()` |

WuYue 更接近 Qiskit 的"寄存器 + 门对象 + 经典控制流"范式，工程完整度高（`QuantumProg` 提供真正的测量条件跳转）；aicir 走**类型化指令**路线：门工厂与 `Circuit.gates` 均为不可变 `Operation`/`Measurement`/`ControlFlow`，运行时（模拟/测量/metrics/transpile 等）经 `aicir.ir` accessor 消费类型化指令，dict 仅在 JSON/QASM/legacy 互操作边界经 `legacy_gates`/`to_gate_dicts()` 出现；`GateSpec` 注册表承担门元信息校验、QASM 名称、绘图符号和分解规则的单一来源。
...
- **aicir**（`core/circuit.py` + `gates/registry.py`）：`pauli_x/cnot/rx/rzz/rxx(ms_gate)/toffoli/u3…` 工厂返回类型化 `Operation`，但保留旧 dict 只读访问并在 `Circuit` 内规范化；受控门用 `(target, control_qubits_list)`，支持 `control_states`（0/1，含 0 控制）与多控制；另有**粒子数守恒激发门**（`single_excitation`/Givens、`double_excitation`），面向量子化学。`GateSpec` 注册表允许集中登记门的目标比特数、参数数、别名、QASM 名、显示符号、生成元与分解/矩阵回退。
...
- **aicir**（`aicir/measure`）：采用统一测量模型，线路内 `measure`/`reset` 是正式操作，末端读出由 `Measure.run(measure_qubits=...)` 控制；二者在 statevector 轨迹路径中可共存。无经典目标的 `measure` 是联合 Pauli 投影，有 `creg`/`cbits` 时是逐比特 Z 基测量并写经典寄存器，供 `if_`/`while_` 逐 shot 执行。`shots=None/0` 为 exact 模式并忽略末端读出；`method="tensor"` 可接入张量网络末态，但拒绝线路内嵌 `measure`/控制流。`estimator` 做基于 shot 的 Pauli 项能量估计（量子比特级对易分组 QWC、基变换测量、shot 分配）；另有 `sampler/projector/trajectory/aggregate`。`primitives` 子包提供 Qiskit 风格 `Sampler`/`Estimator`（Statevector/Shot/Noisy 变体 + BackendSampler/Estimator）。
...
- **WuYue**：`circuit/qasm2.py` 支持 **QASM2 导入**，`QuantumCircuit.QASM()`/`QuantumProg.QASM()` 可导出 QASM2（部分门、批量线路、未赋值参数、用户门受限）；云端 `plugin/runner.py` 内置将电路转 QASM 并用 qiskit/pennylane/cirq 处理 U3 分解、pi 表达式替换等，工程化地对接第三方与真机。
...
- **VQD / SSVQE** 激发态变分算法
- `Operation`/`Measurement`/`ControlFlow` 类型化指令 + `GateSpec` 门元信息层（旧 dict 经 `legacy_gates`/`to_gate_dicts()` 只读兼容）
- OpenQASM **3.0** 往返 + 显式 Qiskit/PennyLane/**WuYue** IO
- 独立 **QUBO 建模** 层、离子阱噪声建模、160 个测试文件的覆盖
...
- 经典控制流（qif/qwhile）+ 经典寄存器（aicir `if_`/`while_` + `ClassicalRegister`，走 `Measure.run` 测量轨迹路径 vs WuYue `qif/trueif/falseif/qwhile` + `QuantumProg`）
...
| 需要经典控制流（测量条件跳转、while 循环） | 均可（aicir `if_`/`while_` / WuYue `qif/qwhile`） |
...
- **aicir** 把赌注押在 *可插拔后端（含国产 NPU）+ 类型化 IR/门元信息演进 + 研究型算法广度（QAS/度量/多变分算法）+ 轻依赖与高测试覆盖*，是做量子算法研究与国产硬件加速的合适底座；张量网络（cotengra 路径+切片）与 UCCSD/化学流水线覆盖了主流模拟方法与化学能力，且独有 NPU 上的可微分张量网络。
