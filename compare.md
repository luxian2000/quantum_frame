...
| 门表示 | 工厂函数返回不可变 `Operation`（带旧 dict 只读兼容），`Circuit.gates` 仍是规范化 dict surface | **门对象**（`Basicgate` 及子类，如 `RX/CU3/RXX`） |
...
WuYue 更接近 Qiskit 的"寄存器 + 门对象 + 经典控制流"范式，工程完整度高（`QuantumProg` 提供真正的测量条件跳转）；aicir 走"类型化 `Operation` + 旧 dict surface 兼容"的轻量路线：用户侧门工厂已返回 `Operation`，内部仍把门规范化为 dict 以保护既有模拟/IO 路径，`GateSpec` 注册表承担门元信息校验、QASM 名称、绘图符号和分解规则的单一来源。
...
- **VQD / SSVQE** 激发态变分算法
- `Operation` + `GateSpec` 类型化门元信息层，同时保持旧 dict surface 兼容
