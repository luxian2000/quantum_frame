...
| 控制流 | `if_`/`while_`（测量反馈，`Measure.run` 执行） | **`qif/trueif/falseif` + `qwhile`**（依赖测量结果的经典条件） |
| 逆电路 | 有 | `dagger()` |
...
- **aicir** 把赌注押在 *可插拔后端（含国产 NPU）+ 研究型算法广度（QAS/度量/多变分算法）+ 轻依赖与高测试覆盖*，是做量子算法研究与国产硬件加速的合适底座；张量网络（cotengra 路径+切片）与 UCCSD/化学流水线覆盖了主流模拟方法与化学能力，且独有 NPU 上的可微分张量网络。
