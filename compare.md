...
| 主打差异 | 后端可插拔（含 **Ascend NPU**）、QAS、丰富 metrics/transpile、多算法编排 | **多种模拟方法**（全/密度矩阵/部分/单振幅/张量网络）、**经典控制流**、**真实量子云 QPU**、PennyLane 风格自动微分 |
| 文档语言 | 中文为主 | 中文为主 |
...
| 张量网络模拟 | ❌ | ✅ `Sim_tc`（TensorNetwork + cotengra 缩并） |
| 单振幅模拟 | ❌ | ✅ `Sim_single` |
| 部分振幅模拟 | ❌ | ✅ `Sim_partial` |
| GPU 后端 | ✅ Torch/CUDA | ⚠️ 经由 qutorch(torch) 间接 |
| **昇腾 NPU 后端** | ✅ `NPUBackend`（自定义 autograd 算子） | ❌ |
...
| torch `nn.Module` 层封装 | ❌（qml 无；仅 QAS 内部用 nn.Module） | ✅ `QuantumLayer(nn.Module)` 可嵌入 PyTorch |
| 量子化学 | 仅固定预置 Hamiltonian（h2 等） | ✅ UCCSD、单/双激发、HF 态、excitations 生成 |
...
这套"按需选择模拟策略"的设计是 WuYue 的核心竞争力，尤其单/部分振幅与张量网络能突破全态矢量的内存墙。

### aicir — 后端抽象 + NPU
aicir 只有两类数值对象（`StateVector` `(2^n,1)`、`DensityMatrix` `(2^n,2^n)`），但把"数值框架"抽象成可插拔 `Backend`：

- `NumpyBackend`（CPU 默认）
- `GPUBackend`（Torch，保留自动微分计算图）
- `NPUBackend`（**昇腾 Ascend**，自定义 `torch.autograd.Function` 算子：矩阵乘、期望值、Hamiltonian 期望、局域门作用）

aicir 没有张量网络/单振幅等替代模拟法，但在**硬件加速**（GPU/NPU）与**自动微分一体化**上更深入。`torchrun` 支持任务级并行（非态分片）。

**结论**：要模拟大而浅/低纠缠线路、或需要单个振幅 → WuYue；要在 GPU/昇腾 NPU 上做可微分变分训练 → aicir。
...
- **aicir `chemistry`**：**仅**固定预置 qubit Hamiltonian（`h2`/`h2_jw`/`h2_tapered`），CLAUDE.md 明确说明"不是电子结构管线"。aicir 的激发门在 `core/gates` 提供，但没有 UCCSD/激发枚举的高层封装。
...
**仅 WuYueSDK 具备：**
- **多模拟方法**：张量网络 / 单振幅 / 部分振幅（突破内存墙）
- **经典控制流**（qif/qwhile）+ 经典寄存器
- **真实量子云 QPU** 接入（ecloud/Runner/WuyueQCloudDevice）
- **UCCSD / 量子化学** 激发算符构造
- `QuantumLayer(nn.Module)` —— 可一行嵌入 PyTorch 的量子层（aicir 有 `@qfun` 装饰器但无此层封装）
...
| 需要模拟大而低纠缠线路 / 只求单个振幅 | **WuYueSDK**（张量网络 / 单/部分振幅） |
| 需要经典控制流（测量条件跳转、while 循环） | **WuYueSDK** |
| 需要把线路跑到真实量子计算机 | **WuYueSDK** |
| 需要 UCCSD 等量子化学激发算符 | **WuYueSDK** |
| 习惯 PennyLane 装饰器风格（两者都有 `qfun`/`qfunc`） | 均可 |
| 要把量子层作为 `nn.Module` 嵌入 PyTorch 混合网络 | **WuYueSDK (qutorch)** |
...
- **aicir** 把赌注押在 *可插拔后端（含国产 NPU）+ 研究型算法广度（QAS/度量/多变分算法）+ 轻依赖与高测试覆盖*，是做量子算法研究与国产硬件加速的合适底座。
- **WuYueSDK** 把赌注押在 *模拟引擎多样性 + 完整电路语义（寄存器/控制流）+ 真实 QPU 云 + 成熟 QML/量子化学栈*，是做工程落地、真机实验与化学/AI 应用的合适底座。
