...
# Ascend NPU verification entrypoints
python demo_npu.py                          # strict NPU check
python demo_npu.py --allow-cpu-fallback     # allow CPU fallback for dev
torchrun --nproc_per_node=2 demo_npu.py     # task-parallel (NOT state sharding)
```
...
- **Gates are plain dicts**, not objects. Gate factories (`pauli_x`, `cnot`, `rx`, `rzz`, `rxx`/`ms_gate`, `toffoli`, `u3`, …) return dicts consumed by `gate_to_matrix` (`aicir/core/gates.py`). Controlled gates take `(target, control_qubits_list)`; dicts may carry `control_states` (0/1) and `control_qubits`.
...
- **Top-level re-exports** (`aicir/__init__.py`): gates, `Circuit`/`Parameter`, backends, `Measure`/`Result`, `PauliOp`/`PauliString`/`Hamiltonian`, noise channels, and QASM/JSON I/O are all importable from `aicir`. `StateVector`/`DensityMatrix` canonical path is `aicir.core` (also re-exported at top level).
