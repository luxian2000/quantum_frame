...
- `aicir/chemistry/` — fixed preset qubit Hamiltonians only, one module per molecule under `chemistry/molecules/`, **filenames in formula case** (`H2.py`/`LiH.py`/`H2O.py`/`NH3.py`/`N2.py`/`BeH2.py`; canonical names lowercase). Presets: `h2`/`h2_jw`/`h2_tapered`, `lih`, `h2o` (≤6q, ground-energy tested) and `nh3`/`n2`/`beh2` (12–16q, structural-guard only — dense diagonalization infeasible). Each self-registers into `MOLECULES` via `register_molecule`. NOT an electronic-structure pipeline; coefficients come from PySCF/Qiskit Nature. Public API unchanged (`get_molecule`/`molecule_hamiltonian`/`molecule_matrix`/… from `aicir.chemistry`).
