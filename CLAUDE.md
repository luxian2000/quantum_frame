...
- `aicir/vqc/` — `BasicVQE`/`run_vqe` orchestration; ansatz templates in `ansatz/` (`hea`, `hea_ti` for trapped-ion, `uccsd` for chemistry — pure-data, decoupled from `aicir.chemistry`, fSWAP network for non-adjacent excitations). Also QAOA/VQD/SSVQE.
